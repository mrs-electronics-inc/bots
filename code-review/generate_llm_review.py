...
def main():
    ...
    change_name = os.getenv('PLATFORM', 'github')
    ...
    change_name = "pull request" if change_name == "github" else "merge request"
    ...
    try:
        ...
        print("Error: System prompt template not found at /bots/system-prompts/review.md", file=sys.stderr)
        sys.exit(1)
    
    # Substitute environment variables in system prompt
    system_prompt = system_prompt_template.replace('${CHANGE_NAME}', change_name)
    ...
    try:
        ...
        print("Error: Context file not found at .bots/context.md", file=sys.stderr)
        sys.exit(1)
    ...
    schema = {
        ...
        "required": ["summary", "raw_change_requests", "change_requests", "feedback"]
    }
    ...
    os.makedirs('.bots/response', exist_ok=True)
    ...
    print("Review generated successfully", file=sys.stderr)
