...
export const issueBotHandler = async (
  ...
  const isIssueNoteEvent = event.event_type === 'note' || event.event_type === 'issue_comment';
...
const getScopedLabelList = async (
  api: IssueBotAPI,
  ...
  return allLabels
    ...
    .filter((name) => name.toLowerCase().includes(scope.toLowerCase() + '::'));
};
...
const checkHasRequiredLabel = async (
  ...
  const scopedLabelList = await getScopedLabelList(api, projectId, labelScope);
  ...
  const hasOneOfRequiredLabels = issue.labels.some((l) => scopedLabelList.includes(l));
